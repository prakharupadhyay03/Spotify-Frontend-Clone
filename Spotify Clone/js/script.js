...
async function getSongs(folder) {
    ...
    songs = []
    let coverImage = "default.jpg"; // fallback image
    ...
    for (let index = 0; index < as.length; index++) {
        ...
        if (
            ...
            coverImage = element.href; // take cover image
        }
    }
    return { songs, coverImage };
}
...
async function main() {
    ...
    async function displayAlbums() {
        ...
        Array.from(document.getElementsByClassName("card")).forEach(e => {
            e.addEventListener("click", async item => {
                let { songs: newSongs, coverImage } = await getSongs(`songs/${item.currentTarget.dataset.folder}`)
                renderPlaylist();
    ...
    next.addEventListener("click", () => {
        ...
        let currentFile = "/" + currentSong.src.split("/").slice(-1)[0]; // add leading "/"
        let index = songs.indexOf(currentFile);
